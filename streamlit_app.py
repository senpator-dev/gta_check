...
def app_filter_dataframe(df, *, search_text: str, geos: list[str], dates: list[str], cabinets: list[str]):
    filtered = df.copy()
    if search_text:
...
def run_streamlit_app() -> None:
    ...
    with tab_filter:
        ...
        with col1:
            search_text = st.text_input("Название содержит", placeholder="Например: SPAIN1, 6T2A")
