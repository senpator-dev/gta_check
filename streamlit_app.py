...
MVP_DEP_SUMMARY_HEADERS = ("dep_summary", "dep summary", "deposit_summary", "deposits", "депозиты", "деп")
MVP_REDEP_SUMMARY_HEADERS = ("redep_summary", "redep summary", "redeposit_summary", "redeposits", "редепозиты", "редеп")
MVP_WEBSITE_CLICK_HEADERS = ("click", "website click", "website clicks", "клики на вебсайте", "клики на сайте")
...
class MvpItem:
    ...
    chat: Decimal
    row_number: int
    dep_summary: Decimal = ZERO
    redep_summary: Decimal = ZERO
    website_clicks: Decimal = ZERO
...
def parse_mvp(path: Path, *, entity: str) -> list[MvpItem]:
    ...
    dep_summary_col = resolve_col(headers, MVP_DEP_SUMMARY_HEADERS)
    redep_summary_col = resolve_col(headers, MVP_REDEP_SUMMARY_HEADERS)
    website_click_col = resolve_col(headers, MVP_WEBSITE_CLICK_HEADERS)
    ...
    for excel_row_number, row in enumerate(rows[header_idx + 1 :], start=header_idx + 2):
        ...
        dep_summary = (
            parse_decimal(cell(row, dep_summary_col), blank_is_zero=True, field=f"dep_summary, строка {excel_row_number}")
            if dep_summary_col is not None else ZERO
        )
        redep_summary = (
            parse_decimal(cell(row, redep_summary_col), blank_is_zero=True, field=f"redep_summary, строка {excel_row_number}")
            if redep_summary_col is not None else ZERO
        )
        website_clicks = (
            parse_decimal(cell(row, website_click_col), blank_is_zero=True, field=f"click, строка {excel_row_number}")
            if website_click_col is not None else ZERO
        )
        output.append(
            MvpItem(
                title=title,
                ...
                redep_summary=redep_summary,
                website_clicks=ensure_nonnegative_integer(website_clicks, field="click", row_number=excel_row_number),
            )
        )
    return output
...
def aggregate_fb_items(items: Iterable[FbItem]) -> list[FbItem]:
    """Склеивает одинаковые FB-названия: spend/clicks/views суммируются."""
    ...
    for item in items:
        ...
        bucket = grouped[key]
        bucket["spend"] = bucket["spend"] + item.spend
        bucket["first_seen_index"] = min(bucket["first_seen_index"], item.first_seen_index)
        bucket["row_number"] = min(bucket["row_number"], item.row_number)

        if bucket["budget"] is None and item.budget is not None:
            bucket["budget"] = item.budget

        if bucket["clicks"] is None and item.clicks is not None:
            bucket["clicks"] = item.clicks
        elif bucket["clicks"] is not None and item.clicks is not None:
            bucket["clicks"] = bucket["clicks"] + item.clicks

        if bucket["views"] is None and item.views is not None:
            bucket["views"] = item.views
        elif bucket["views"] is not None and item.views is not None:
            bucket["views"] = bucket["views"] + item.views
    ...
    return [
        ...
        for bucket in sorted(grouped.values(), key=lambda x: x["first_seen_index"])
    ]


def aggregate_mvp_items(items: Iterable[MvpItem]) -> list[MvpItem]:
    """Склеивает одинаковые MVP-названия: sub/chat/dep/redep/click суммируются."""
    grouped: dict[str, dict[str, object]] = {}
    for item in items:
        key = item.normalized_title
        if key not in grouped:
            grouped[key] = {
                ...
                "chat": item.chat,
                "row_number": item.row_number,
                "dep_summary": item.dep_summary,
                "redep_summary": item.redep_summary,
                "website_clicks": item.website_clicks,
            }
            continue

        bucket = grouped[key]
        bucket["sub"] = bucket["sub"] + item.sub
        bucket["chat"] = bucket["chat"] + item.chat
        bucket["row_number"] = min(bucket["row_number"], item.row_number)
        bucket["dep_summary"] = bucket["dep_summary"] + item.dep_summary
        bucket["redep_summary"] = bucket["redep_summary"] + item.redep_summary
        bucket["website_clicks"] = bucket["website_clicks"] + item.website_clicks
    ...
    return [
        MvpItem(
            ...
            chat=bucket["chat"],
            row_number=bucket["row_number"],
            dep_summary=bucket["dep_summary"],
            redep_summary=bucket["redep_summary"],
...
def build_check(
    ...
    *,
    ...
    fb_items = aggregate_fb_items(fb_items)
    mvp_items = aggregate_mvp_items(mvp_items)
...
def build_app_context(
    ...
    *,
    ...
    fb_items_raw, resolved_entity = parse_fb(fb_path, entity=entity, include_zero_spend=False)
    mvp_items_raw = parse_mvp(mvp_path, entity=resolved_entity) if mvp_path else []
    fb_items = aggregate_fb_items(fb_items_raw)
    mvp_items = aggregate_mvp_items(mvp_items_raw)
    ...
    for item in fb_items:
        ...
        sub = mvp.sub if mvp else ZERO
        chat = mvp.chat if mvp else ZERO
        dep_summary = mvp.dep_summary if mvp else ZERO
        redep_summary = mvp.redep_summary if mvp else ZERO
        deposits_total = dep_summary + redep_summary
        website_clicks = mvp.website_clicks if mvp else ZERO
        ...
        rows.append(
            {
                ...
                "Депозиты": decimal_to_float(deposits_total),
                "dep_summary": decimal_to_float(dep_summary),
                "redep_summary": decimal_to_float(redep_summary),
                "Клики на вебсайте": decimal_to_int_or_float(website_clicks),
                "Цена ПДП": decimal_to_float(pdp_price),
                "Цена диа": decimal_to_float(dialog_price),
                "Клики FB": decimal_to_int_or_float(item.clicks),
            ...
            rows.append(
                {
                    ...
                    "Депозиты": decimal_to_float(item.dep_summary + item.redep_summary),
                    "dep_summary": decimal_to_float(item.dep_summary),
                    "redep_summary": decimal_to_float(item.redep_summary),
                    "Клики на вебсайте": decimal_to_int_or_float(item.website_clicks),
                    "Цена ПДП": None,
                    "Цена диа": None,
                    "Клики FB": None,
                    "Просмотры": None,
...
def app_smart_name_match(title: object, needle: str) -> bool:
    """Умный contains: ищет фразу в названии, но не внутри цельного слова/блока.

    Пример:
    - needle "ПОПА" совпадёт с "19 - ПОПА - 2"
    - needle "ПОПА" НЕ совпадёт с "19 - ПОПА3 - 3"
    - needle "SPAIN" НЕ совпадёт с "SPAIN-ES", потому что дефис внутри блока считается частью токена
    """
    needle_norm = normalize_spaces(needle).upper()
    title_norm = normalize_spaces(title).upper()
    if not needle_norm:
        return True

    # Буквы, цифры, подчёркивание и дефис считаем частью одного блока.
    # Поэтому "ПОПА" не совпадает с "ПОПА3", а "SPAIN" не совпадает с "SPAIN-ES".
    token_chars = r"0-9A-ZА-ЯЁ_\\-"
    pattern = rf"(?<![{token_chars}]){re.escape(needle_norm)}(?![{token_chars}])"
    return re.search(pattern, title_norm, flags=re.IGNORECASE) is not None


def app_filter_dataframe(
    df,
    *,
    search_text: str = "",
    ...
    active_names = [normalize_spaces(name) for name in (names or []) if normalize_spaces(name)]
    if search_text:
        active_names.append(normalize_spaces(search_text))

    if active_names:
        mask = filtered["Название"].fillna("").apply(
            lambda title: any(app_smart_name_match(title, name) for name in active_names)
        )
...
def app_add_pinned_name(input_key: str, list_key: str) -> None:
    import streamlit as st

    value = normalize_spaces(st.session_state.get(input_key, ""))
    if not value:
        return
    current = st.session_state.setdefault(list_key, [])
    if value not in current:
        current.append(value)
    st.session_state[input_key] = ""


def app_pinned_names_control(*, list_key: str, input_key: str, title: str = "Название") -> list[str]:
    import streamlit as st

    st.text_input(
        title,
        key=input_key,
        placeholder="Введи название и нажми Enter, например ПОПА",
        on_change=app_add_pinned_name,
        args=(input_key, list_key),
    )

    pinned = st.session_state.setdefault(list_key, [])
    if pinned:
        st.caption("Закреплённые названия:")
        for index, name in enumerate(list(pinned)):
            col_name, col_remove = st.columns([8, 1])
            col_name.code(name)
            if col_remove.button("×", key=f"{list_key}_remove_{index}", help="Убрать это название"):
                pinned.pop(index)
                st.rerun()
        if st.button("Очистить названия", key=f"{list_key}_clear"):
            st.session_state[list_key] = []
            st.rerun()

    return list(st.session_state.get(list_key, []))
...
def run_streamlit_app() -> None:
    ...
    st.caption("Загрузи FB и MVP Excel — приложение соберёт чек, покажет расхождения и посчитает суммы по фильтрам. Дубли одинаковых названий автоматически суммируются.")
    ...
    with tab_filter:
        ...
        st.caption("Можно закрепить несколько названий через Enter и считать сумму по ним вместе. Поиск работает как «содержит», но не цепляет куски внутри одного слова: ПОПА не найдёт ПОПА3.")

        col1, col2 = st.columns(2)
        with col1:
            names = app_pinned_names_control(
                list_key="sum_pinned_names",
                input_key="sum_name_input",
                title="Название",
            )
        ...
        fb_clicks_sum = filtered["Клики FB"].dropna().sum() if "Клики FB" in filtered and filtered["Клики FB"].notna().any() else None
        website_clicks_sum = filtered["Клики на вебсайте"].fillna(0).sum() if "Клики на вебсайте" in filtered else 0
        ...
        deposits_sum = filtered["Депозиты"].fillna(0).sum() if "Депозиты" in filtered else 0

        metric_cols = st.columns(8)
        metric_cols[0].metric("Найдено строк", len(filtered))
        metric_cols[1].metric("Расход", app_money_metric(spend_sum))
        metric_cols[2].metric("Клики FB", app_number_metric(fb_clicks_sum))
        metric_cols[3].metric("Просмотры", app_number_metric(views_sum))
        metric_cols[4].metric("Клики на вебсайте", app_number_metric(website_clicks_sum))
        metric_cols[5].metric("ПДП", app_number_metric(sub_sum))
        metric_cols[6].metric("Диа", app_number_metric(chat_sum))
        metric_cols[7].metric("Депозиты", app_money_metric(deposits_sum))
        ...
        show_cols = [
            "Статус", "Название", "Гео", "Дата", "Бюджет",
            "Расход", "ПДП", "Диа", "Депозиты", "dep_summary", "redep_summary",
            "Клики на вебсайте", "Цена ПДП", "Цена диа", "Клики FB", "Просмотры",
        ]
    ...
    with tab_search:
        ...
        quick_query = st.text_input("Поиск", placeholder="Введи часть названия или гео", key="quick_search")
        ...
        quick_cols = ["Статус", "Название", "Гео", "Дата", "Расход", "ПДП", "Диа", "Депозиты", "Клики на вебсайте", "Клики FB", "Просмотры"]
    ...
    with tab_mismatch:
        ...
        if mismatches.empty:
            ...
            mismatch_cols = ["Статус", "Название", "Гео", "Дата", "Расход", "ПДП", "Диа", "Депозиты", "Клики на вебсайте", "FB строка", "MVP строка"]
