...
def app_title_matches_any(title: object, terms: Sequence[str], *, mode: str) -> bool:
    """Проверяет название по одному или нескольким закреплённым фильтрам."""
    normalized_terms = [normalize_spaces(term).upper() for term in terms if normalize_spaces(term)]
    if not normalized_terms:
        return True

    title_text = normalize_spaces(title).upper()
    title_blocks = split_title_blocks(str(title or ""))
    ...
    for term in normalized_terms:
        if mode == "contains":
            ...
            # Точное совпадение = совпадение с отдельным блоком названия через " - ".
            # Поэтому фильтр SPAIN совпадает с "... - SPAIN - ...", но не с "... - SPAIN-ES - ...".
            if term in title_blocks:
                return True
        else:
            raise ValueError(f"Некорректный режим фильтра названия: {mode}")
    return False
...
def app_filter_dataframe(
    ...
    *,
    ...
    # Старый быстрый поиск оставлен как обычное "содержит".
    if search_text:
    ...
    # Новый фильтр вкладки "Сумма по фильтру": несколько закреплённых названий + режим.
    if name_filters:
        filtered = filtered[
            filtered["Название"].apply(lambda title: app_title_matches_any(title, name_filters, mode=name_match_mode))
        ]
...
def app_add_name_filter_term(input_key: str, terms_key: str) -> None:
    import streamlit as st

    raw_value = st.session_state.get(input_key, "")
    term = normalize_spaces(raw_value)
    if not term:
        return

    terms = list(st.session_state.get(terms_key, []))
    normalized_existing = {normalize_spaces(item).upper() for item in terms}
    if term.upper() not in normalized_existing:
        terms.append(term)
    st.session_state[terms_key] = terms
    st.session_state[input_key] = ""


def app_remove_name_filter_term(terms_key: str, index_to_remove: int) -> None:
    import streamlit as st

    terms = list(st.session_state.get(terms_key, []))
    if 0 <= index_to_remove < len(terms):
        del terms[index_to_remove]
    st.session_state[terms_key] = terms
    st.rerun()


def app_clear_name_filter_terms(terms_key: str) -> None:
    import streamlit as st
...
def run_streamlit_app() -> None:
    ...
    with tab_filter:
        ...
        st.caption("Фильтры можно комбинировать: название, гео, дата и кабинет/T2A.")

        name_terms_key = "filter_name_terms"
        name_input_key = "filter_name_input"
        if name_terms_key not in st.session_state:
            st.session_state[name_terms_key] = []

        col1, col2, col3 = st.columns(3)
        with col1:
            name_mode_label = st.selectbox(
                "Режим названия",
                options=("Точное совпадение", "Содержит"),
                index=0,
                help="Точное совпадение ищет отдельный блок названия через ' - '. SPAIN не захватит SPAIN-ES.",
            )
            name_match_mode = "exact" if name_mode_label == "Точное совпадение" else "contains"
            st.text_input(
                "Название",
                placeholder="Напиши SPAIN и нажми Enter",
                key=name_input_key,
                on_change=app_add_name_filter_term,
                args=(name_input_key, name_terms_key),
            )

            name_terms = list(st.session_state.get(name_terms_key, []))
            if name_terms:
                st.caption("Закреплённые названия. Фильтр считает строки, где найдено хотя бы одно из них.")
                for idx, term in enumerate(name_terms):
                    term_cols = st.columns([0.82, 0.18])
                    term_cols[0].markdown(f"`{term}`")
                    if term_cols[1].button("✕", key=f"remove_name_filter_{idx}"):
                        app_remove_name_filter_term(name_terms_key, idx)
                if st.button("Очистить названия", key="clear_name_filters"):
                    app_clear_name_filter_terms(name_terms_key)
            else:
                st.caption("После Enter название закрепится, и можно будет добавить второе, третье и т.д.")
    ...
    with tab_search:
        ...
        quick_query = st.text_input("Поиск", placeholder="Введи часть названия, гео или T2A", key="quick_search")
        ...
        quick_cols = ["Статус", "Название", "Гео", "Дата", "Кабинет/T2A", "Расход", "ПДП", "Диа", "Клики", "Просмотры"]
    ...
    with tab_mismatch:
        ...
        if mismatches.empty:
            ...
            mismatch_cols = ["Статус", "Название", "Гео", "Дата", "Кабинет/T2A", "Расход", "ПДП", "Диа", "FB строка", "MVP строка"]
