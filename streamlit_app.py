...
def app_filter_dataframe(
    ...
    *,
    ...
    if active_names:
        title_series = filtered["Название"].fillna("").map(normalize_spaces)
        title_upper = title_series.str.upper()
        mask = False
        for name in active_names:
            needle = normalize_spaces(name).upper()
            if not needle:
                continue
            if name_mode == "exact":
                current_mask = title_upper == needle
            else:
                current_mask = title_upper.str.contains(re.escape(needle), regex=True)
...
def app_pinned_names_control(*, list_key: str, input_key: str, title: str = "Название") -> list[str]:
    ...
    st.text_input(
        title,
        key=input_key,
        placeholder="Введи название и нажми Enter",
...
def run_streamlit_app() -> None:
    ...
    with tab_filter:
        ...
        st.caption("Можно закрепить несколько названий через Enter и считать сумму по ним вместе. Названия работают в режиме «точное совпадение» или «содержит».")
        ...
        with col1:
            name_mode = st.radio(
                ...
                index=0,
                format_func=lambda value: "содержит" if value == "contains" else "точное совпадение",
                horizontal=True,
                key="sum_name_mode",
            )
